...
const app = express();
const URL_DATABASE = "mongodb+srv://javi18pm:iMdX5mZcNnx1ojRA@sd-agencia.8nzbs.mongodb.net/Vehiculos?retryWrites=true&w=majority";
...
app.get('/api', (req, res, next) => {
    ...
    db.getCollectionNames((err, vehiculos) => {
...
// Obtenemos todos los elementos de la tabla :coleccion (Ofertas o Reservas de Vehiculos).
...
app.get('/api/:coleccion/search', (req, res, next) => {
    ...
    db.collection(queColeccion).find({marca: request.marca, 
...
// Reservar
app.post('/api/reserva/vehiculo/:id', auth, (req, res, next) => {
    const queID = req.params.id;
    ...
    db.collection("Ofertas").findOne(id(queID), (err, elemento) => {
        ...
        if(elemento != null){
            if(elemento.reservado != true){
                ...
                res.status(400).json({
                    error: 'El vehiculo ya se encuentra reservado'
                });
            }
        }
        else {
...
// Cancelar reserva
app.delete('/api/reserva/vehiculo/:id', auth, (req, res, next) => {
    const queID = req.params.id;
    const queUser = req.body.email;
    var reserva;
        ...
        if(elemento != null){
            if(elemento.reservado == true) {
                ...
                if(elemento.usuario == queUser){
                    ...
                    res.status(400).json({
                        error: 'No se puede cancelar la reserva del vehiculo',
...
https.createServer(opciones, app). listen(port, () => {
    console.log(`WS API RESTful Proveedor de Vehiculos ejecutándose en https://localhost:${port}/api/:coleccion`);
});

...
app.post('/api/reservar', auth, async (req, res, next) => {
    ...
    var bookError = false; //controlar si alguna reserva a fallado
    ...
    if(user == null){ //si el usuario no existe, abortar.
        ...
        if((vuelo != null || vehiculo != null || hotel != null) && !conectionFailed && !bookError){ // Si hay alguna reserva realizado y no ha fallado ni la conexión ni una reserva...
            ...
            if(conectionFailed) { // Si la conexión a la pasarela a fallado ...
...
app.delete('/api/reservar', auth, async (req, res, next) => {
    ...
    if(vuelo != null){
        const getURL = `${URL_WS_vuelos}/reserva/vuelo/${vuelo}`;
        res1 = await fetch(getURL, {method: 'DELETE', body: JSON.stringify(req.body), headers: {'Content-Type': 'application/json','Authorization': `Bearer ${queToken}`}}).then(res => res.json())
            //Mi lógica de negocio...
            .then(json => {
                res1 = ({
                result: json
    ...
    if(vehiculo != null){
        const getURL = `${URL_WS_vehiculos}/reserva/vehiculo/${vehiculo}`;
        res2 = await fetch(getURL, {method: 'DELETE', body: JSON.stringify(req.body), headers: {'Content-Type': 'application/json','Authorization': `Bearer ${queToken}`}}).then(res => res.json())
            //Mi lógica de negocio...
            .then(json => {
                res2 = ({
                result: json
    ...
    if(hotel != null){
        const getURL = `${URL_WS_hoteles}/reserva/hotel/${hotel}`;
        res3 = await fetch(getURL, {method: 'DELETE', body: JSON.stringify(req.body), headers: {'Content-Type': 'application/json','Authorization': `Bearer ${queToken}`}}).then(res => res.json())
            //Mi lógica de negocio...
            .then(json => {
                res3 = ({
                result: json
